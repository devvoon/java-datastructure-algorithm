...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //위치 제한 N(0 ≤ N ≤ 100,000) 이 있음 
        //1번이동하는데 1초이기때문 
        int[] arr = new int[100001];         //이전 정점에서 현재 정점까지의 시간 계산
        bfs(arr, n, k);
    ...
    public static void bfs(int[] arr, int start, int end) {
        ...
        while(!queue.isEmpty()) {
           ...
           if ( (now+1) < 100001 && arr[now+1] == 0) {
               queue.add(now+1);
               arr[now+1] = arr[now] + 1;
            }
           
           // x*2  이동 : 이동 노드 값이 최대값 미만, 방문한적이 없어야 함
           if ( now*2 < 100001 && arr[now*2] == 0) {
               queue.add(now*2);
               arr[now*2] = arr[now] + 1;
            }
        }
